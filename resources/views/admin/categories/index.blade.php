@extends('layouts.admin')

@section('title', 'Categorias - Painel Administrativo')

@section('content')
<div class="page-header d-flex justify-content-between align-items-center">
    <div>
        <h1>
            <i class="bi bi-folder me-2"></i>Categorias
        </h1>
        <p>Gerencie as categorias do blog</p>
    </div>
    <a href="{{ route('admin.categories.create') }}" class="btn btn-primary">
        <i class="bi bi-plus-circle me-2"></i>Nova Categoria
    </a>
</div>

{{-- Mensagens serão exibidas via Toast --}}

<!-- Gráficos -->
@if($chartData->count() > 0)
<div class="row mb-4">
    <div class="col-lg-6">
        <div class="admin-card">
            <div class="card-header">
                <h5>
                    <i class="bi bi-bar-chart me-2"></i>Distribuição de Posts por Categoria
                </h5>
            </div>
            <div class="card-body">
                <canvas id="categoriesBarChart" height="300"></canvas>
            </div>
        </div>
    </div>
    <div class="col-lg-6">
        <div class="admin-card">
            <div class="card-header">
                <h5>
                    <i class="bi bi-pie-chart me-2"></i>Percentual de Uso
                </h5>
            </div>
            <div class="card-body">
                <canvas id="categoriesPieChart" height="300"></canvas>
            </div>
        </div>
    </div>
</div>
@endif

<div class="admin-card">
    <div class="table-responsive">
        <table class="table table-hover align-middle">
            <thead>
                <tr>
                    <th>#</th>
                    <th>Nome</th>
                    <th>Slug</th>
                    <th>Descrição</th>
                    <th>Posts</th>
                    <th>Ações</th>
                </tr>
            </thead>
            <tbody>
                @forelse($categories as $category)
                <tr>
                    <td>{{ $category->id }}</td>
                    <td>
                        <strong>{{ $category->name }}</strong>
                    </td>
                    <td>
                        <code class="text-muted">{{ $category->slug }}</code>
                    </td>
                    <td>
                        <small class="text-muted">{{ Str::limit($category->description ?? 'Sem descrição', 50) }}</small>
                    </td>
                    <td>
                        <span class="badge bg-info">{{ $category->posts_count }} post(s)</span>
                    </td>
                    <td>
                        <div class="d-flex gap-2">
                            <a href="{{ route('admin.categories.edit', $category) }}" class="btn btn-sm btn-outline-primary" title="Editar">
                                <i class="bi bi-pencil"></i>
                            </a>
                            @if($category->posts_count > 0)
                            <form action="{{ route('admin.categories.forceDestroy', $category) }}" method="POST" class="d-inline" 
                                  onsubmit="return confirm('Tem certeza? Esta categoria está atrelada a {{ $category->posts_count }} post(s). O atrelamento será removido.');">
                                @csrf
                                @method('DELETE')
                                <button type="submit" class="btn btn-sm btn-outline-danger" title="Excluir (removerá atrelamento)">
                                    <i class="bi bi-trash"></i>
                                </button>
                            </form>
                            @else
                            <form action="{{ route('admin.categories.destroy', $category) }}" method="POST" class="d-inline" 
                                  onsubmit="return confirm('Tem certeza que deseja excluir esta categoria?');">
                                @csrf
                                @method('DELETE')
                                <button type="submit" class="btn btn-sm btn-outline-danger" title="Excluir">
                                    <i class="bi bi-trash"></i>
                                </button>
                            </form>
                            @endif
                        </div>
                    </td>
                </tr>
                @empty
                <tr>
                    <td colspan="6" class="text-center py-4">
                        <p class="text-muted mb-0">Nenhuma categoria encontrada.</p>
                        <a href="{{ route('admin.categories.create') }}" class="btn btn-sm btn-primary mt-2">
                            <i class="bi bi-plus-circle me-1"></i>Criar Primeira Categoria
                        </a>
                    </td>
                </tr>
                @endforelse
            </tbody>
        </table>
    </div>

    @if($categories->hasPages())
    <div class="d-flex justify-content-center mt-4">
        {{ $categories->links() }}
    </div>
    @endif
</div>

@push('styles')
<style>
    .admin-card .card-body canvas {
        max-height: 300px;
    }
    @media (max-width: 991.98px) {
        .row.mb-4 > div {
            margin-bottom: 1rem;
        }
    }
</style>
@endpush

@push('scripts')
@if($chartData->count() > 0)
<script src="https://cdn.jsdelivr.net/npm/chart.js@4.4.0/dist/chart.umd.min.js"></script>
<script>
    // Dados do gráfico
    const categoriesData = @json($chartData);
    
    // Cores para os gráficos
    const colors = [
        '#e63946', '#457b9d', '#a8dadc', '#f1faee', '#1d3557',
        '#ff6b6b', '#4ecdc4', '#ffe66d', '#95e1d3', '#f38181',
        '#aa96da', '#fcbad3', '#ffffd2', '#a8e6cf', '#dcedc1'
    ];

    // Gráfico de Barras
    const barCtx = document.getElementById('categoriesBarChart').getContext('2d');
    new Chart(barCtx, {
        type: 'bar',
        data: {
            labels: categoriesData.map(cat => cat.name),
            datasets: [{
                label: 'Número de Posts',
                data: categoriesData.map(cat => cat.posts_count),
                backgroundColor: colors.slice(0, categoriesData.length),
                borderColor: colors.slice(0, categoriesData.length).map(c => c + 'dd'),
                borderWidth: 2,
                borderRadius: 8
            }]
        },
        options: {
            responsive: true,
            maintainAspectRatio: false,
            plugins: {
                legend: {
                    display: false
                },
                tooltip: {
                    callbacks: {
                        label: function(context) {
                            return context.parsed.y + ' post(s)';
                        }
                    }
                }
            },
            scales: {
                y: {
                    beginAtZero: true,
                    ticks: {
                        stepSize: 1
                    }
                }
            }
        }
    });

    // Gráfico de Pizza
    const pieCtx = document.getElementById('categoriesPieChart').getContext('2d');
    new Chart(pieCtx, {
        type: 'pie',
        data: {
            labels: categoriesData.map(cat => cat.name),
            datasets: [{
                data: categoriesData.map(cat => cat.posts_count),
                backgroundColor: colors.slice(0, categoriesData.length),
                borderColor: '#ffffff',
                borderWidth: 2
            }]
        },
        options: {
            responsive: true,
            maintainAspectRatio: false,
            plugins: {
                legend: {
                    position: 'right',
                    labels: {
                        padding: 15,
                        usePointStyle: true
                    }
                },
                tooltip: {
                    callbacks: {
                        label: function(context) {
                            const label = context.label || '';
                            const value = context.parsed || 0;
                            const total = context.dataset.data.reduce((a, b) => a + b, 0);
                            const percentage = ((value / total) * 100).toFixed(1);
                            return `${label}: ${value} post(s) (${percentage}%)`;
                        }
                    }
                }
            }
        }
    });
</script>
@endif
@endpush
@endsection

