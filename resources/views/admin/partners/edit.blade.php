...
<div class="page-header">
    <div>
        <h1>
            <i class="bi bi-handshake me-2"></i>Editar Parceiro
        </h1>
        <p>Atualize as informações do parceiro</p>
    </div>
</div>

<form action="{{ route('admin.partners.update', $partner) }}" method="POST" enctype="multipart/form-data" id="partnerForm">
    @csrf
    @method('PUT')
        ...
        <div class="col-lg-4">
            <div class="admin-card mb-4">
                ...
                <div class="card-body">
                    @if($partner->logo)
                    <div class="mb-3">
                        <label class="form-label fw-semibold">Logo Atual</label>
                        <div>
                            <img src="{{ $partner->logo }}" alt="Logo atual" 
                                 class="img-fluid rounded mb-2" style="max-height: 150px; width: 100%; object-fit: contain;">
                        </div>
                    </div>
                    @endif

                    <div class="mb-3">
                        <label for="logo" class="form-label fw-semibold">Novo Logo</label>
                ...
                <div class="card-body">
                    <div class="mb-3">
                        ...
                        <select class="form-select @error('level') is-invalid @enderror" 
                            ...
                            <option value="gold" {{ old('level', $partner->level) == 'gold' ? 'selected' : '' }}>Ouro</option>
                            <option value="silver" {{ old('level', $partner->level) == 'silver' ? 'selected' : '' }}>Prata</option>
                            <option value="bronze" {{ old('level', $partner->level) == 'bronze' ? 'selected' : '' }}>Bronze</option>
                        </select>
                        @error('level')
                            <div class="invalid-feedback">{{ $message }}</div>
                        @enderror
